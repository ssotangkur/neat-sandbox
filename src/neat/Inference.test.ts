...
describe("Inference", () => {
  it("when no hidden nodes, predicts", () => {
    const nn = createSimpleNeuralNet(2, 1);
    ...
    expect(kernel.predict([0, 0, 0])).toEqual([0]);
    expect(kernel.predict([1, 1, 0])).toEqual([2]);
    expect(kernel.predict([1, 0, 0])).toEqual([1]);
    expect(kernel.predict([0, 1, 0])).toEqual([1]);
  });
  ...
  it("when one hidden node predicts", () => {
    ...
    const in1 = nn.neurons[0];
    const out = nn.neurons[3];
    const hidden = new Neuron(nn.maxIdNeuron.id + 1, "hidden", "sigmoid");
    nn.neurons.push(hidden);
    nn.genes.push(new Gene(in1.id, hidden.id, 1));
    nn.genes.push(new Gene(hidden.id, out.id, 1));
    const kernel = new Kernel(nn);

    expect(kernel.predict([0, 0, 0])).toEqual([0.5]);
    expect(kernel.predict([1, 1, 0])).toEqual([0.75]);
    expect(kernel.predict([1, 0, 0])).toEqual([0.75]);
    expect(kernel.predict([0, 1, 0])).toEqual([0.5]);
  });
  ...
  it("when one hidden node and multiple connections", () => {
    ...
    expect(kernel.predict([0, 0, 0])).toEqual([0.5]);
    expect(kernel.predict([1, 1, 0])[0]).toBeCloseTo(0.833333333333333);
    expect(kernel.predict([1, 0, 0])).toEqual([0.75]);
    expect(kernel.predict([0, 1, 0])).toEqual([0.75]);
  });
